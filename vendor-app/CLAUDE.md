...
- `src/lib/` — pure, unit-tested helpers: `totals` (round2, discount), `format` (baht), `receipt`, `backup` (CSV export + JSON backup/restore), `promptpay` (EMVCo QR payload, fully offline), `ads` (Capacitor AdMob, no-op on web).
