...
- Store listing screenshots come from `node scripts/device-shots.mjs [outDir]` against an Android device or emulator — 8 shots at 1080x1920 carrying the real system status bar and gesture bar, which is what Play listings should show. `scripts/store-shots.mjs` is the browser-only fallback (same 8 frames, no Android chrome) for when no device is attached.
- The device script's header lists the adb prep it needs; three of those steps are not optional. `svc power stayon true` — it drives the app over CDP and sends zero touch events, so the screen sleeps, the app is backgrounded and the system kills it. `pm clear` — a shift left open by an earlier run turns the "เปิดกะ" button into "ปิดกะ" and the run dies on a missing selector. And it stubs AdMob at the Capacitor bridge (`nativePromise`/`toNative`), because leaving the Sale tab calls `showBanner()` and the SDK's binder chatter gets the process killed mid-capture — logcat says "excessive binder traffic during cached". Playwright cannot drive this target at all: Android WebView answers `connectOverCDP` with "Browser context management is not supported", hence the hand-rolled CDP client. Do not eyeball the geometry: 9:16 is 0.5625 exactly and Play rejects anything outside 16:9..9:16, so a 1200x2134 raster (0.56232) is already too tall. The script seeds its own member/promo straight into IndexedDB, opens a shift before selling so the drawer figures are not all zero, collapses the AdMob banner spacer (it photographs as an empty white strip on web), and captures `#root` rather than the viewport because headless Chromium's viewport raster ghosts the bottom nav near the top of a freshly switched tab.
